defmodule SimpleCrudWeb.UserAuthTest do
  use SimpleCrudWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias SimpleCrud.Accounts
  alias SimpleCrud.Accounts.Scope
  alias SimpleCrudWeb.UserAuth

  import SimpleCrud.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, SimpleCrudWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      SimpleCrudWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        SimpleCrudWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
